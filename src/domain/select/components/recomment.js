...
import { deleteRecommentThunk, editRecommentThunk } from "../../../app/module/commentSlice";
import { fontExtraBold, fontMedium, fontSmall } from "../../../shared/themes/textStyle";
...
const Recomment = (state) => {
  ...
  //본인확인
  const auth = state.state?.userKey === parseInt(localStorage.getItem("userKey"));
      ...
      <RecommentContainer>
        <RecommentBox>
          <MakeItRow>
            ...
            <Buttons>
              {auth ? editMode ? null : <Delete onClick={onClickDelete}>삭제</Delete> : null}

              {auth ? editMode ? null : <Edit onClick={onClickEditMode}>수정</Edit> : null}
            </Buttons>
          </MakeItRow>
          <Comment>
            {editMode ? (
              <EditInput maxLength="50" onChange={onChangeEdit} value={editComment} />
            ) : (
              state.state.comment
            )}
          </Comment>
