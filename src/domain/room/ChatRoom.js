...
import { ModalBasic, ModalRecommend, ModalExit, ModalKick } from "common/components/Modal";

import { IconLarge, IconSmall } from "shared/themes/iconStyle";
import { fontBold, fontExtraBold } from "shared/themes/textStyle";
import { fontExtraSmall, fontLarge, fontMedium, fontSmall } from "shared/themes/textStyle";
...
const ChatRoom = () => {
  ...
  return (
    <>
            ...
            <StPeopleInfo>
              ...
              {nowUsers.map((user) => (
                <StPersonInfo key={user.userKey}>
                  ...
                  {user.userKey === Number(userKey) && <StMeBadge>나</StMeBadge>}
                  <StPersonNickname isMe={user.userKey === Number(userKey)}>
                    {user.nickname}
                  </StPersonNickname>
                  {roomInfo?.userKey === Number(userKey) && user.userKey !== Number(userKey) && (
                    <StKickBadge
                      onClick={() => {
                        setModalKick(true);
                        setMainModal(false);
      ...
      {hostByeModal && (
        <ModalBasic setter={() => navigate("/main", { state: { now: "room" }, replace: true })}>
          {hostByeModal}
        </ModalBasic>
      )}
